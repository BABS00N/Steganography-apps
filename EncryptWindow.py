...
def encrypt():
    ...
    keyArea.delete(1.0, END)
    keyArea.insert("end", Encryptor.getKey)

# def back():
#     winEncr.destroy()
#     import Shifrator
#----------------------------
textArea_lb = tk.Label(winEncr, text='Шифруемый текст (eng)',
                bg='gray',
                fg='white',
                font=('Arial',15),
                padx=34,
...
keyArea_label = Label(winEncr, text='Ключ от зашифрованного изображения:',
                bg='gray',
                fg='white',
                font=('Arial',11),
                padx=35,
...
keyArea = tk.Text(winEncr, height=1,width=53,font="Arial 11")
...
winEncr.grid_rowconfigure(6,minsize=50)
winEncr.grid_rowconfigure(7,minsize=30)
winEncr.grid_rowconfigure(8,minsize=10)
winEncr.grid_rowconfigure(9,minsize=150)
...
errorLabel.grid(row=6,column=0,columnspan=2,stick='s')
keyArea_label.grid(row=7,column=0,columnspan=2,stick='nw')
keyArea.grid(row=8,column=0,columnspan=2)
#buttonBack.grid(row=9,column=1,stick='se')
#----------------------------
winEncr.mainloop()
